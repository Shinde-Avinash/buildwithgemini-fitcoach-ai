...
def get_memory_service():
    """Process-wide memory bank service shared across every serving surface."""
    ...
    if agent_engine_id := os.environ.get(
        "GOOGLE_CLOUD_AGENT_ENGINE_ID", "1380399465277423616"
    ):
        from google.adk.memory import VertexAiMemoryBankService

        return VertexAiMemoryBankService(
            project=os.environ.get("GOOGLE_CLOUD_PROJECT", "qwiklabs-gcp-01-426fafba1fca"),
            location=os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_LOCATION")
            or os.environ.get("GOOGLE_CLOUD_LOCATION")
            or "us-east1",
            agent_engine_id=agent_engine_id,
